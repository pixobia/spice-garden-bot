...
- **`Order.items` is JSON.** Shape: `[{ itemId: number, quantity: number, priceAtTime: number }]`. Total is `sum(quantity * priceAtTime) + (items.length > 0 ? deliveryFee : 0)`, computed in the order service whenever items change. Empty cart pays no delivery fee. Cached in `subtotal`/`total` columns so reads (admin DM, customer summary) don't reparse.
